...
//
// ⚠️ DUPLICATED VERBATIM in sqrz-dashboard/app/lib/heroTransform.ts — these two
// repos are separate codebases, so the copies must be kept byte-for-byte in sync.
// The picker (sqrz-dashboard) and the hero render (sqrz-profiles) both call this
// so the crop the artist sees can never drift from the crop the visitor sees.
//
